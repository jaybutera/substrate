...
use rstd::prelude::*;
use rstd::result;
use primitives::traits::{Zero, As, Staking};
...
// TODO TODO: pub crate ?
pub type BalanceOf<T> = <<T as Trait>::Staking as Staking<<T as system::Trait>::AccountId>>::Balance;

pub trait Trait: system::Trait + Sized {
	type Staking: Staking<<Self as system::Trait>::AccountId>;
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn second(origin, #[compact] proposal: PropIndex) {
			...
			T::Staking::reserve(&who, deposit.0)
		...
		fn vote(origin, #[compact] ref_index: ReferendumIndex, vote: Vote) {
			...
			ensure!(!T::Staking::total_balance(&who).is_zero(),
...
impl<T: Trait> Module<T> {
	...
	fn bake_referendum(now: T::BlockNumber, index: ReferendumIndex, info: ReferendumInfo<T::BlockNumber, T::Proposal>) -> Result {
		...
		let total_issuance = T::Staking::total_issuance();
